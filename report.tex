...
In order to accomplish this, we created an PostgreSQL script that processes
...
too strange to include} and all relevant items.
...
including metadata about the item's usage, and the \texttt{item\_tag} table (2828 rows)
...
takes on a mostly consistent format, but the input section differs dramatically depending
on the ``type'' of the recipe.
...
We start by putting all JSON records into two files in CSV format (using \texttt{src/mk-csv.sh}). This is the extent
of processing done outside SQL. From here, at the request of our instructor, we proceed
